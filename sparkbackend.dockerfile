...
RUN curl --retry 2 -L -s -o /tmp/examples_artifacts.tar.gz https://github.com/bullet-db/bullet-db.github.io/releases/download/v${BULLET_EXAMPLES_VERSION}/examples_artifacts.tar.gz
RUN tar -xzf /tmp/examples_artifacts.tar.gz -C /tmp/examples_artifacts
RUN mv /tmp/examples_artifacts/bullet-examples/backend/spark/bullet-spark-example.jar /opt/bitnami/spark/jars/
RUN rm -rf /tmp/examples_artifacts*
...
COPY configs/spark-backend/bullet_spark_kafka_settings.yaml /opt/bitnami/spark/conf/bullet_spark_kafka_settings.yaml
